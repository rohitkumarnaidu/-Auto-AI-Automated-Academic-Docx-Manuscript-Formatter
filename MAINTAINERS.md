# Maintainers

## Core Team

| Name | Role | Area | GitHub | Contact |
|------|------|------|--------|---------|
| Rohit Kumar Naidu | BDFL / Project Lead | Overall direction, architecture | [@rohitkumarnaidu](https://github.com/rohitkumarnaidu) | rohit@scholarform.ai |
| _(open)_ | Core Team | Backend, pipeline, AI | @user | — |
| _(open)_ | Core Team | Frontend, UX | @user | — |
| _(open)_ | Core Team | DevOps, infrastructure | @user | — |

## Committers

| Name | Area | GitHub |
|------|------|--------|
| _(vacant)_ | — | — |

## Emeritus

Past maintainers who have stepped down but remain involved as advisors.

| Name | Former Role | GitHub |
|------|-------------|--------|
| _(none)_ | — | — |

## Maintainer Responsibilities

- Review pull requests within 48 hours (core business days)
- Triage new issues within 72 hours
- Participate in release coordination
- Maintain project documentation and CI/CD
- Mentor new contributors

## Becoming a Maintainer

See [GOVERNANCE.md](GOVERNANCE.md) for the nomination and voting process.

---

*Last updated: June 2026*
