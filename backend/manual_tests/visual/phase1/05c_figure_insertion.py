"""
Visual Test - Stage 5c: Figure Insertion
Purpose: Verify figure insertion anchor detection visually
Input: DOCX file
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python 05c_figure_insertion.py <input.docx>")
        sys.exit(1)
        
    input_path = sys.argv[1]
    output_dir = Path(__file__).parent.parent.parent / "visual_outputs"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "05c_figure_insertion_annotated.docx"
    
    print("=" * 70)
    print("VISUAL TEST - STAGE 5c: FIGURE INSERTION")
    print("=" * 70)
    ...
    doc_obj = parser.parse(input_path, "visual_test_figins")
    ...
    for i, fig in enumerate(doc_obj.figures, 1):
        ...
        if fig.caption_block_id:
            caption_block = next((b for b in doc_obj.blocks if b.block_id == fig.caption_block_id), None)
            if caption_block:
    ...
    summary.add_run("=== FIGURE INSERTION SUMMARY ===\n").bold = True
    ...
    for block in doc_obj.blocks:
        ...
        if fig:
            ...
            add_comment_to_paragraph(para, f"FIG {fig.number} → INSERT AT {anchor_idx}")
