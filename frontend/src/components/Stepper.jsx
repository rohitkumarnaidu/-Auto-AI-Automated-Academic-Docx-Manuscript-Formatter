                    ...
                    <div key={index} className={`flex items-start gap-4 ${isPending ? 'opacity-50' : ''}`}>
                        <div className="flex flex-col items-center">
                            ...
                            {index < STEPS.length - 1 && (
                                <div className="w-0.5 h-8 mt-2 bg-slate-200 dark:bg-slate-800 relative overflow-hidden">
                                    {isCompleted && (
                                        <div className="absolute inset-0 bg-green-500" style={{ animation: 'sf-fade-in 300ms ease-out both' }} />
                                    )}
                                </div>
                            )}
                        </div>
